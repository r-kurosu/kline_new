...
def Read_booking(FileName):
    ...
    # 港番号のエンコード
    Check_port = []
    port_name = Booking["LPORT"].unique()
    key1 = Booking.columns.get_loc('CPORT')
    for j in range(len(Booking)):
        count = 0
        if str(Booking.iloc[j, key1]) != 'nan':
            for p in port_name:
                if p == str(Booking.iloc[j, key1]):
                    Check_port.append(count)
                else:
                    count = count + 1

    list_cols = ['LPORT', 'DPORT']
    ...
    Booking_transform = ce_oe.fit_transform(Booking)
    Port = Booking_transform.iloc[:, key2:key3+1] - 1
    ...
    for j in J:
        if U[j] <= 75:
            J_small.append(j)
        if 75 < U[j] and U[j] <= 200:
            J_medium.append(j)
        if 200 < U[j]:
            J_large.append(j)

    return J, U, A, G, J_small, J_medium, J_large, Port, Check_port, Booking, divide_dic
...
def main():
    ...
    # 注文情報の読み込み
    J, U, A, G, J_small, J_medium, J_large, Port, Check_port, Booking, divide_dic \
        = Read_booking(File1)
    ...
    L_port = []
    key = Booking.columns.get_loc('LPORT')
    max = 0
    for i in range(len(Port)):
        l = Port.iloc[i, 0]
        if l not in T:
            T.append(l)
            L_port.append([l, Booking.iloc[i, key]])
            if max < l:
                max = l
    # print(L_port)
    ...
    for i in range(len(Port)):
        ...
        if d_tmp not in T:
            T.append(d_tmp)
            D_port.append([d_tmp, Booking.iloc[i, key]])
    # print(D_port)

    for k in range(len(L_port)):
        L.append(L_port[k][0])

    for k in range(len(D_port)):
        D.append(D_port[k][0])
    ...
    for t in T:
        ...
        for t_k in range(N):
            if t > tmp_load[t_k] and t < tmp_dis[t_k] + k + 1:
                J_keep.append(t_k)
                lk.append(t_k)
    ...
    for i1 in I:
        for i2 in I:
            ...
            if i1 != i2:
                if check1 in I_next or check2 in I_next:
                    GAP_SP.addConstrs(X_ij[i1,j] + X_ij[i2,j] <= 2 for j in J_medium)
                else:
                    GAP_SP.addConstrs(X_ij[i1,j] + X_ij[i2,j] <= 1 for j in J_medium)
    """
    ...
    """
    #ギャング数を考えて指定台数で分ける
    for k in range(len(J_large)):
        
        if len(J_large_divide[k]) == 2:
            gang_low = GANG2[0]
            gang_high = GANG2[1]
            j = J_large[k]
            
            GAP_SP.addConstr(gp.quicksum(V_ij[i,j] for i in gang_low) == J_large_divide[k][0])
            GAP_SP.addConstr(gp.quicksum(V_ij[i,j] for i in gang_high) == J_large_divide[k][1])
            ...
            for i1 in gang_low:
                for i2 in gang_low:
                    ...
                    if i1 != i2:
                        if check1 in I_next or check2 in I_next:
                            GAP_SP.addConstr(X_ij[i1,j] + X_ij[i2,j] <= 2)
                        else:
                            GAP_SP.addConstr(X_ij[i1,j] + X_ij[i2,j] <= 1)
        ...
        if len(J_large_divide[k]) == 3:
            ...
            GAP_SP.addConstr(gp.quicksum(V_ij[i,j] for i in gang_low) == J_large_divide[k][0])
            GAP_SP.addConstr(gp.quicksum(V_ij[i,j] for i in gang_mid) == J_large_divide[k][1])
            GAP_SP.addConstr(gp.quicksum(V_ij[i,j] for i in gang_high) == J_large_divide[k][2])
    ...
    for k in range(len(assign)):
        ...
        I_left_data.iloc[answer[k][0],
                         1] = I_left_data.iloc[answer[k][0], 1] - assign[k][7]
    ...
    assign_data.to_excel('result/exp_assignment.xlsx',
                         index=False, columns=c_list)
    I_left_data.to_excel('result/exp_leftRT.xlsx', index=False)
