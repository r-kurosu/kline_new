        ...
        for col2 in D:
            ...
            for k in range(len(Booking_df)):
                if Booking_df.iloc[k,key2] == col1 and Booking_df.iloc[k,key3] == col2:
                    ...
                    weight += Booking_df.iloc[k,key4] * Booking_df.iloc[k,key6]
...
def main():
    ...
    #注文情報の読み込み
    T,L,D,J,U,A,G,J_small,J_large,Port,Check_port,Booking \
    = Read_booking(File1)
    ...
    for i in I:
        for j in J:
            V_ij[i,j] = GAP_SP.addVar(lb = 0, ub = A[j], vtype = gp.GRB.CONTINUOUS, name = f"V_ij({i},{j})")
    ...
    OBJ3 = gp.quicksum(w3 * penal3_m * M_ijt[i,j,t] for i in I for j in J for t in T)  
    ...
    #目的関数3の制約
    for t in T:
        GAP_SP.addConstrs(M_it[i,t] >= - Stress[i] + (gp.quicksum(V_ij[i,j] for j in J_t_keep[t]) / B[i]) for i in I)
        ...
        for i1 in I:
            ...
            GAP_SP.addConstrs(M_ijt[i1,j,t] >= gp.quicksum(M_it[i2,t] for i2 in I_prime) for j in J_ld[t])
    
    #目的関数4の制約
    for t in check_point:
        GAP_SP.addConstrs(N_it[i,t] <= B[i] - gp.quicksum(V_ij[i,j] for j in J_lk[t]) for i in I)
    
    #目的関数5の制約
    GAP_SP.addConstrs(K1_it[i,t] >= - 0.75 + gp.quicksum(V_ij[i,j] for j in J_lk[t]) / B[i] for i in I_lamp for t in L)
    GAP_SP.addConstrs(K2_it[i,t] >= 1 - (gp.quicksum(V_ij[i,j] for j in J_lk[t]) + 1) / B[i] for i in I for t in L)
    ...
    #10RT以下に分割しない
    GAP_SP.addConstrs(V_ij[i,j] + v_min * (1 - X_ij[i,j]) >= v_min for i in I for j in J)
    ...
    for t in T:
        for i1 in I:
            ...
            for k in range(N_prime):
                i2 = int(I_prime[k])
                GAP_SP.addConstrs(gp.quicksum(V_ij[i2,j1] for j1 in J_t_keep[t]) / B[i2]  <= 1 + Fr[k] - X_ij[i1,j2] for j2 in J_ld[t])
    ...
    for t in T:
        ...
        #荷物を全て降ろしたとき
        GAP_SP.addConstr(gp.quicksum(delta_h[i] * U[j] * G[j] * V_ij[i,j] / A[j] for j in J_t_keep[t] for i in I) <= max_h)
        GAP_SP.addConstr(gp.quicksum(delta_s[i] * U[j] * G[j] * V_ij[i,j] / A[j] for j in J_t_keep[t] for i in I) <= max_s)
        GAP_SP.addConstr(gp.quicksum(delta_s[i] * U[j] * G[j] * V_ij[i,j] / A[j] for j in J_t_keep[t] for i in I) >= min_s)
        
        #荷物を全て載せたとき
        GAP_SP.addConstr(gp.quicksum(delta_h[i] * U[j] * G[j] * V_ij[i,j] / A[j] for j in J_lk[t] for i in I) <= max_h)
        GAP_SP.addConstr(gp.quicksum(delta_s[i] * U[j] * G[j] * V_ij[i,j] / A[j] for j in J_lk[t] for i in I) <= max_s)
        GAP_SP.addConstr(gp.quicksum(delta_s[i] * U[j] * G[j] * V_ij[i,j] / A[j] for j in J_lk[t] for i in I) >= min_s)
    ...
    print(f"The number of cars passed holds that exceed threshold : {penal3_m} × {penal3}")
    ...
    for k in range(len(assign)):
        ...
        #残リソース計算
        I_left_data.iloc[answer[k][0], 1] = I_left_data.iloc[answer[k][0], 1] - assign[k][2]
