...
def main():
    ...
    # 注文情報の読み込み
    T, L, D, J, U, A, G, J_small, J_medium, J_large, Port, Check_port, Booking, divide_dic, VehicleHeight = Read_booking(
        BookingFile)
    ...
    # 船体情報の読み込み2
    Ml_Load, Ml_Back, Ml_Afr, Stress, GANG2, GANG3 = Read_other(
        MainLampFile, BackMainLampFile, AfrMainLampFile, StressFile, Gang2File, Gang3File, Hold_encode)
    ...
    # Gurobiパラメータ設定
    GAP_SP = gp.Model()
    GAP_SP.setParam("TimeLimit", 36000)
    ...
    try:
        ...
        print(
            f"The number of cars passed holds that exceed threshold : {penal3}")
        print(
        ...
        for k in range(len(assign)):
            ...
            # order_ID
            assign[k][1] = Booking.iloc[j_t,
                                        Booking.columns.get_loc('Order_num')]
        ...
        c_list.append("DPORT")
        c_list.append("Cost")
        booking_name = BookingFile.split(".")[0].split("/")[-1]
        assignment_result_name = "result/without_height_"+booking_name+"_assignment.xlsx"
        leftRT_result_name = "result/without_height_"+booking_name+"_leftRT.xlsx"
        assign_data = pd.DataFrame(assign, columns=c_list)
        assign_data.to_excel(assignment_result_name,
